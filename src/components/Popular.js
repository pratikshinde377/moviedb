...
import { API_KEY, BASE_API, fetchPopuarMovies, img_url } from "../api";
...
function Popular(props) {
  ...
  const fetchLoadMore = async (page) => await BASE_API.get(`/movie/popular?api_key=${API_KEY}&page=${page}&language=en-US`).then((response) => {
  ...
  return (<>
    <h1 className='offset-1'>Popular Movies</h1>
        ...
        <div className="container d-flex flex-wrap allign-item-space-between">
          {
            ...
            data?.map((item, index) => (
              console.log(item),
              <Grid style={{ justifyContent: "space-around", padding: "5px" }}  >
